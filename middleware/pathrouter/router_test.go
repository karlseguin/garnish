...
func TestRouterReturnsNilIfRouteNotFound(t *testing.T) {
	...
	route, res := buildRouter().router(garnish.ContextBuilder().SetRequest(req))
...
func TestRouterMatchesRoot(t *testing.T) {
	...
	route, res := buildRouter("/", "root").router(garnish.ContextBuilder().SetRequest(req))
...
func TestRoutesMatchesANestedRoute(t *testing.T) {
	...
	route, res := buildRouter("/", "root", "/houses/", "houses", "/houses/*/mentant/", "mentant").router(garnish.ContextBuilder().SetRequest(req))
...
func TestRoutesMatchesATheClosestMatch(t *testing.T) {
	spec := gspec.New(t)
	req := gspec.Request().Url("/houses/atreides/mentant").Req
	route, res := buildRouter("/", "root", "/houses/", "houses").router(garnish.ContextBuilder().SetRequest(req))
...
func TestRouterMatchesASimpleRoute(t *testing.T) {
	...
	route, res := buildRouter("/houses", "up1").router(garnish.ContextBuilder().SetRequest(req))
	spec.Expect(route.Upstream).ToEqual("up1")
	spec.Expect(res).ToBeNil()
}

func buildRouter(data ...string) *Router {
	config := Configure(garnish.Configure(nil))

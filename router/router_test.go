...
func TestRouterReturnsNilIfRouteNotFound(t *testing.T) {
	...
	route, _, res := buildRouter().Route(garnish.ContextBuilder().SetRequestIn(req))
...
func TestRouterMatchesRoot(t *testing.T) {
	...
	route, _, res := buildRouter("/", "root").Route(garnish.ContextBuilder().SetRequestIn(req))
...
func TestRoutesMatchesANestedRoute(t *testing.T) {
	...
	route, params, res := buildRouter("/", "root", "/houses/", "houses", "/houses/:houseName/mentant/", "mentant").Route(garnish.ContextBuilder().SetRequestIn(req))
...
func TestRoutesMatchesANestedRouteEndingWithAParameter(t *testing.T) {
	...
	route, params, res := buildRouter("/", "root", "/houses/", "houses", "/houses/:houseName", "showHouse").Route(garnish.ContextBuilder().SetRequestIn(req))
...
func TestRoutesMatchesANestedRouteAFailedConstraint(t *testing.T) {
	...
	route, _, res := r.Route(garnish.ContextBuilder().SetRequestIn(req))
	spec.Expect(route.Name).ToEqual("root")
	spec.Expect(res).ToBeNil()
}
...
func TestRoutesMatchesATheClosestMatch(t *testing.T) {
	...
	route, _, res := buildRouter("/", "root", "/houses/", "houses").Route(garnish.ContextBuilder().SetRequestIn(req))
...
func TestRouterMatchesASimpleRoute(t *testing.T) {
	...
	route, _, res := buildRouter("/houses", "up1").Route(garnish.ContextBuilder().SetRequestIn(req))
